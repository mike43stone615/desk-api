...
complianceGatewayRouter.all('/*', async (c) => {
  ...
  if (!config.complianceOsUrl) throw new ApiError(503, 'Compliance service is not configured.');
  return proxyUpstream(c.req.raw, config.complianceOsUrl, c.req.path, new URL(c.req.raw.url).search);
});
...
registryGatewayRouter.all('/*', async (c) => {
  ...
  return proxyUpstream(c.req.raw, config.registryApiUrl, c.req.path, new URL(c.req.raw.url).search);
});

async function proxyUpstream(req: Request, targetBase: string, path: string, search: string): Promise<Response> {
  ...
  const skip = new Set(['host', 'connection', 'transfer-encoding', 'te', 'upgrade', 'keep-alive']);
