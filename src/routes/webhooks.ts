...
// RESEND_WEBHOOK_SECRET configured the endpoint answers 404, and a wrong or stale signature is a 401.
...
export function registerWebhooks(app: FastifyInstance): void {
  ...
  void app.register(async (scope) => {
    ...
    scope.post('/webhooks/resend', { bodyLimit: 65_536 }, async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const ok = typeof h['svix-id'] === 'string' && typeof h['svix-timestamp'] === 'string' && typeof h['svix-signature'] === 'string'
        && verifySvixSignature(secret, h['svix-id'], h['svix-timestamp'], raw, h['svix-signature']);
