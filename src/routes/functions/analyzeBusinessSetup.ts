...
const GENERIC_CLASSIFICATION_KEYWORDS = new Set(['business', 'company', 'service', 'services', 'startup', 'venture']);
...
function keywordMatches(lower: string, keyword: string): boolean {
  ...
  if (words.length === 1) {
    ...
    return new RegExp(`(?<![a-z0-9])${escapeRegExp(word)}${suffix}(?![a-z0-9])`).test(lower);
  }
  const body = words.map(escapeRegExp).join('[\\s/&+.-]+');
  return new RegExp(`(?<![a-z0-9])${body}(?![a-z0-9])`).test(lower);
}
...
function keywordMatchScore(lower: string, rule: FallbackClassificationRule): number {
  return rule.keywords.reduce((score, keyword) => {
    ...
    const normalized = keyword.toLowerCase().trim();
    if (GENERIC_CLASSIFICATION_KEYWORDS.has(normalized)) return score + 0.25;
