...
export async function analyzeBusinessSetupHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Analyze an unregistered business setup. Return JSON only with classification, marketValidation, businessPlanSections, ideaIsPlausible, ideaFeedback. classification must contain targetMarket, industry, geographicScope, customerType. industry must exactly match one allowedIndustries value. geographicScope must be Local or National. customerType must be B2B, B2C, or Both. ' +
    ...
    return reply.send({
      ...
      marketValidation: normalizeMarketValidation(parsed.marketValidation, businessIdea, industries),
      businessPlanSections: normalizeBusinessPlanSections(parsed.businessPlanSections, businessIdea, industries, body),
...
function normalizeClassification(input: Record<string, unknown>, idea: string, industries: string[]): Classification {
...
function fallback(idea: string, industries: string[]): Classification {
  ...
  const geographicScope: 'Local' | 'National' =
    lower.includes('online') || lower.includes('national') || lower.includes('software') || lower.includes('app') || lower.includes('content')
      ? 'National'
      : 'Local';
  const customerType: 'B2B' | 'B2C' | 'Both' =
    lower.includes('business') || lower.includes('b2b') || lower.includes('company')
      ...
      : lower.includes('consumer') || lower.includes('family') || lower.includes('home')
        ...
        : 'Both';
  return {
    targetMarket: 'Customers most likely to need the proposed product or service',
    industry,
    geographicScope,
    customerType,
  };
...
function fallbackEnrichment(idea: string, industries: string[], body: Record<string, unknown>) {
  ...
  return {
    classification,
    marketValidation,
    businessPlanSections: fallbackBusinessPlanSections(idea, classification, marketValidation, body),
    ideaIsPlausible: plausibility.isPlausible,
    ideaFeedback: plausibility.feedback,
    source: 'fallback',
  };
...
function assessIdeaPlausibilityHeuristic(idea: string): IdeaPlausibility {
  ...
  const looksLikeGibberish =
    !trimmed.includes(' ') && letters.length >= GIBBERISH_MIN_LETTERS && vowels / letters.length < GIBBERISH_VOWEL_RATIO;
  if (looksLikeGibberish) {
    return {
      isPlausible: false,
      feedback: "This doesn't look like a business idea yet. Describe what the business would do in a few words.",
    };
  }
...
function normalizeIdeaPlausibility(input: Record<string, unknown>, idea: string): IdeaPlausibility {
  ...
  const feedback =
    typeof input.ideaFeedback === 'string' && input.ideaFeedback.trim() ? input.ideaFeedback.trim() : heuristic.feedback;
  return { isPlausible: false, feedback };
}

function normalizeMarketValidation(input: unknown, idea: string, industries: string[]): MarketValidation {
...
function normalizeBusinessPlanSections(
  ...
  const classification = fallback(idea, industries);
  const marketValidation = fallbackMarketValidation(idea, classification);
  if (!Array.isArray(input)) return fallbackBusinessPlanSections(idea, classification, marketValidation, body);
  const sections = input
    .map((item) => (item && typeof item === 'object' ? (item as Record<string, unknown>) : null))
    .filter((item): item is Record<string, unknown> => item !== null)
    .map((item) => ({ title: String(item.title ?? '').trim(), content: String(item.content ?? '').trim() }))
    .filter((section) => section.title && section.content);
  return sections.length >= 8 ? sections : fallbackBusinessPlanSections(idea, classification, marketValidation, body);
}
...
function fallbackBusinessPlanSections(
  ...
  const partners = body.hasPartners ? `${Number(body.numberOfPartners ?? 1)} partner(s)` : 'one owner';
  return [
    {
    ...
    },
    {
      ...
      content: 'Define the launch offer, what is included, what is excluded, delivery timeline, service standards, and later expansion opportunities.',
    },
    {
      title: 'Business Model And Pricing',
      content: 'Add revenue model, planned price points, payment terms, expected margin, break-even volume, and recurring revenue opportunities.',
    },
    {
    ...
    },
    {
      ...
      content: 'Document workflow, suppliers, tools, scheduling, staffing, quality control, customer support, insurance, and recordkeeping.',
    },
    {
    ...
    },
    {
      ...
      content: 'Add startup costs, monthly fixed costs, variable costs, sales forecast, gross margin, owner pay, taxes, cash reserve, funding needs, and break-even assumptions.',
    },
    {
      title: 'Milestones And Metrics',
      content: 'Track validation, formation, bank account, licenses, launch, first customers, break-even, leads, conversion rate, order value, margin, repeat rate, reviews, and cash runway.',
    },
    {
      title: 'Risks And Mitigation',
      content: 'List demand, licensing, cost, supplier, regulatory, cash-flow, and capacity risks. Add early warning signs and mitigation actions.',
    },
  ];
}
