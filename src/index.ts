...
import { complianceGatewayRouter, registryGatewayRouter } from './api/routes/api-gateway.js';
...
// ── Health ────────────────────────────────────────────────────────────────────
app.get('/health', (c) => {
  return c.json({ ok: true, service: 'desk-api', ts: new Date().toISOString() });
});
...
app.get('/readiness', async (c) => {
  try {
    ...
    console.error(JSON.stringify({ level: 'error', event: 'readiness_failed', requestId: c.get('requestId'), err: String(err) }));
...
app.onError((err, c) => {
  ...
  if (err instanceof Error) {
    ...
    console.error(JSON.stringify({ level: 'error', event: 'request_error', requestId, message: err.message }));
  }
