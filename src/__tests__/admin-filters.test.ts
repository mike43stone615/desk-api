...
import { parseFilters, escapeLikeValue, buildFilterClause } from '../api/routes/admin.js';

describe('parseFilters', () => {
  it('keeps only whitelisted columns with non-empty trimmed values', () => {
    const raw = JSON.stringify({ email: '  test@example.com  ', not_a_column: 'x', status: '' });
    expect(parseFilters(raw, ['email', 'status'])).toEqual({ email: 'test@example.com' });
  });
