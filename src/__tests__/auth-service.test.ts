...
import {
  DeskAuthService,
  AuthError,
} from "../infrastructure/auth/auth-service.js";
import type {
  DatabaseRepository,
...
function makeEmailConfirmationToken(
  overrides: Partial<EmailConfirmationToken> = {},
): EmailConfirmationToken {
  return {
    id: "confirm-1",
    userId: "user-1",
...
function makeDb(
  ...
  return {
    findUserById: vi
      .fn<DatabaseRepository["findUserById"]>()
      .mockResolvedValue(null),
    findUserByEmail: vi
      .fn<DatabaseRepository["findUserByEmail"]>()
      .mockResolvedValue(null),
    createUser: vi
      .fn<DatabaseRepository["createUser"]>()
      .mockImplementation(
        async (id, email, passwordHash, firstName, lastName) =>
          makeUser({ id, email, passwordHash, firstName, lastName }),
      ),
    updateUserPassword: vi
      .fn<DatabaseRepository["updateUserPassword"]>()
      .mockResolvedValue(undefined),
    markUserEmailConfirmed: vi
      .fn<DatabaseRepository["markUserEmailConfirmed"]>()
      .mockResolvedValue(undefined),
    createSession: vi
      .fn<DatabaseRepository["createSession"]>()
      .mockImplementation(async (id, userId, token, expiresAt) =>
        makeSession({ id, userId, token, expiresAt }),
      ),
    findSessionByToken: vi
      .fn<DatabaseRepository["findSessionByToken"]>()
      .mockResolvedValue(null),
    deleteSession: vi
      .fn<DatabaseRepository["deleteSession"]>()
      .mockResolvedValue(undefined),
    deleteExpiredSessions: vi
      .fn<DatabaseRepository["deleteExpiredSessions"]>()
      .mockResolvedValue(undefined),
    createResetToken: vi
      .fn<DatabaseRepository["createResetToken"]>()
      .mockResolvedValue(undefined),
    findResetToken: vi
      .fn<DatabaseRepository["findResetToken"]>()
      .mockResolvedValue(null),
    findLatestResetTokenForUser: vi
      .fn<DatabaseRepository["findLatestResetTokenForUser"]>()
      .mockResolvedValue(null),
    markResetTokenUsed: vi
      .fn<DatabaseRepository["markResetTokenUsed"]>()
      .mockResolvedValue(undefined),
    deleteExpiredPasswordResetTokens: vi
      .fn<DatabaseRepository["deleteExpiredPasswordResetTokens"]>()
      .mockResolvedValue(undefined),
    createEmailConfirmationToken: vi
      .fn<DatabaseRepository["createEmailConfirmationToken"]>()
      .mockResolvedValue(undefined),
    findEmailConfirmationToken: vi
      .fn<DatabaseRepository["findEmailConfirmationToken"]>()
      .mockResolvedValue(null),
    findLatestEmailConfirmationTokenForUser: vi
      .fn<DatabaseRepository["findLatestEmailConfirmationTokenForUser"]>()
      .mockResolvedValue(null),
    markEmailConfirmationTokenUsed: vi
      .fn<DatabaseRepository["markEmailConfirmationTokenUsed"]>()
      .mockResolvedValue(undefined),
    deleteExpiredEmailConfirmationTokens: vi
      .fn<DatabaseRepository["deleteExpiredEmailConfirmationTokens"]>()
      .mockResolvedValue(undefined),
    ...overrides,
  };
}
...
describe("DeskAuthService.signUp", () => {
  ...
  it("throws email_in_use when email already exists", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signUp("test@example.com", "Password1!", "Test", "User"),
  ...
  it("throws password_too_short for passwords under 8 characters", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signUp("new@example.com", "abc", "New", "User"),
  ...
  it("throws for missing password complexity requirements", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signUp("new@example.com", "password1!", "New", "User"),
    ).rejects.toMatchObject({ code: "password_missing_uppercase" });
    await expect(
      svc.signUp("new@example.com", "PASSWORD1!", "New", "User"),
    ).rejects.toMatchObject({ code: "password_missing_lowercase" });
    await expect(
      svc.signUp("new@example.com", "Password!", "New", "User"),
    ).rejects.toMatchObject({ code: "password_missing_number" });
    await expect(
      svc.signUp("new@example.com", "Password1", "New", "User"),
    ).rejects.toMatchObject({ code: "password_missing_symbol" });
  });
  ...
  it("throws when names are missing", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signUp("new@example.com", "Password1!", "", "User"),
    ).rejects.toMatchObject({ code: "first_name_required" });
    await expect(
      svc.signUp("new@example.com", "Password1!", "New", ""),
    ).rejects.toMatchObject({ code: "last_name_required" });
  });
  ...
  it("rejects empty password", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signUp("new@example.com", "", "New", "User"),
    ).rejects.toMatchObject({ code: "password_too_short" });
  });
});
...
describe("DeskAuthService.signIn — NEEDS_RESET sentinel", () => {
  it("throws password_reset_required when user has NEEDS_RESET sentinel", async () => {
    const db = makeDb({
      findUserByEmail: vi
        .fn()
        .mockResolvedValue(makeUser({ passwordHash: "NEEDS_RESET" })),
    });
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signIn("test@example.com", "anypassword"),
...
describe("DeskAuthService.signIn", () => {
  it("throws email_not_confirmed for valid credentials before confirmation", async () => {
    ...
    const db = makeDb({
      findUserByEmail: vi
        .fn()
        .mockResolvedValue(makeUser({ passwordHash, emailConfirmedAt: null })),
    });
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.signIn("test@example.com", "Mypassword1!"),
...
describe("DeskAuthService.requestPasswordReset", () => {
  ...
  it("returns null and skips creating a token when the last one was issued within the cooldown", async () => {
    const db = makeDb({
      ...
      findLatestResetTokenForUser: vi
        .fn()
        .mockResolvedValue(
          makeResetToken({ createdAt: new Date().toISOString() }),
        ),
    });
  ...
  it("issues a new token once the cooldown window has passed", async () => {
    ...
    const db = makeDb({
      ...
      findLatestResetTokenForUser: vi
        .fn()
        .mockResolvedValue(makeResetToken({ createdAt: staleCreatedAt })),
    });
    const svc = new DeskAuthService(db, 720, 60, 60 * 24, 60);
    const token = await svc.requestPasswordReset("test@example.com");
    expect(token).not.toBeNull();
...
describe("DeskAuthService.requestEmailConfirmation", () => {
  it("returns a token string for an unconfirmed user", async () => {
    const db = makeDb({
      findUserByEmail: vi
        .fn()
        .mockResolvedValue(makeUser({ emailConfirmedAt: null })),
    });
    const svc = new DeskAuthService(db, 720, 60);
  ...
  it("returns null for an already-confirmed user", async () => {
    const db = makeDb({
      findUserByEmail: vi
        .fn()
        .mockResolvedValue(
          makeUser({ emailConfirmedAt: new Date().toISOString() }),
        ),
    });
    const svc = new DeskAuthService(db, 720, 60);
  ...
  it("returns null and skips creating a token when the last one was issued within the cooldown", async () => {
    const db = makeDb({
      ...
      findLatestEmailConfirmationTokenForUser: vi
        .fn()
        .mockResolvedValue(
          makeEmailConfirmationToken({ createdAt: new Date().toISOString() }),
        ),
    });
    const svc = new DeskAuthService(db, 720, 60, 60 * 24, 60);
    expect(await svc.requestEmailConfirmation("test@example.com")).toBeNull();
  ...
  it("issues a new token once the cooldown window has passed", async () => {
    ...
    const db = makeDb({
      ...
      findLatestEmailConfirmationTokenForUser: vi
        .fn()
        .mockResolvedValue(
          makeEmailConfirmationToken({ createdAt: staleCreatedAt }),
        ),
    });
    const svc = new DeskAuthService(db, 720, 60, 60 * 24, 60);
    const token = await svc.requestEmailConfirmation("test@example.com");
...
describe("DeskAuthService.confirmPasswordReset", () => {
  it("updates password and marks token used for a valid token", async () => {
    ...
    expect(db.markResetTokenUsed).toHaveBeenCalledWith(
      "reset-tok",
      expect.any(String),
    );
  });
  ...
  it("returns false for unknown token", async () => {
    const db = makeDb();
    const svc = new DeskAuthService(db, 720, 60);
    expect(await svc.confirmPasswordReset("bad-token", "Newpassword1!")).toBe(
      false,
    );
  });
  ...
  it("returns false for already-used token", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    expect(await svc.confirmPasswordReset("reset-tok", "Newpassword1!")).toBe(
      false,
    );
  });
  ...
  it("throws password_too_short for weak password", async () => {
    ...
    const svc = new DeskAuthService(db, 720, 60);
    await expect(
      svc.confirmPasswordReset("reset-tok", "abc"),
    ).rejects.toMatchObject({ code: "password_too_short" });
  });
});
...
describe("DeskAuthService.updatePassword", () => {
  ...
  it("throws password_too_short for short password", async () => {
    const db = makeDb();
    const svc = new DeskAuthService(db, 720, 60);
    await expect(svc.updatePassword("user-1", "hi")).rejects.toMatchObject({
...
describe("DeskAuthService.confirmEmail", () => {
  it("marks the user and token confirmed for a valid token", async () => {
    ...
    expect(ok).toBe(true);
    expect(db.markUserEmailConfirmed).toHaveBeenCalledWith(
      "user-1",
      expect.any(String),
    );
    expect(db.markEmailConfirmationTokenUsed).toHaveBeenCalledWith(
      "confirm-tok",
      expect.any(String),
    );
  });
