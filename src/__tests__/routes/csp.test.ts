...
describe('the library web pages', () => {
  it('send the library policy instead, under the configured header name, and no other policy', async () => {
    for (const url of ['/', '/login', '/developer', '/app.js', '/style.css', '/pages/auth.js', '/pages/developer.js', '/developer/teams', '/pages/teams.js', '/developer/webhooks', '/developer/apps', '/developer/billing', '/pages/webhooks.js', '/pages/apps.js', '/pages/billing.js', '/developer/admin', '/pages/admin.js']) {
      const res = await app.inject({ method: 'GET', url });
